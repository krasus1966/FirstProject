...
public class EditProduct extends JFrame {
    ...
    public void add() {   // 商品条形码
        ...
        unit_input.setText(product.getUnit());
        ...
        save.addActionListener(new ActionListener() {
            ...
            @Override
            public void actionPerformed(ActionEvent e) {     // 1、商品条形码
                product.setIsbn(isbn_input.getText());     // 2、商品名称
                product.setName(name_input.getText());     // 3、商品价格
                product.setSale_price(new BigDecimal(price_input.getText()));     // 4、商品单位
                product.setUnit(unit_input.getText());     // 5、保存到数据库     // 获取实现类对象
                ProductDao productDao = new ProductDaoImpl();
                try {
                    productDao.update(product);     // 返回采购主界面
